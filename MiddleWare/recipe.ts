...
import { AllRecipeJSON, FileJSON, RecipeDetail, RecipeJSON } from '../interface';
...
class _recipe {
    ...
    if(!check_number(seq)) {
      ...
      try{
    this.connection.query(`SELECT recipeName,rarity,summary from Recipe WHERE seq='${seq}'`, (error:mysql.MysqlError, rows:any) => {
     if (error) {
    ...
    upload = (req:Request, res:Response, next:NextFunction) => {
        ...
        let data:RecipeJSON = {     //업로드 데이터
         ...
         if(!check_name(data.recipeName) || !check_name(data.rarity) || !check_name(data.summary)) {
             res.status(404).end(); 
             return;
         }

        
           this.connection.query(`INSERT INTO Recipe (recipeName, rarity, summary) VALUES ('${data.recipeName}', '${data.rarity}', '${data.summary}'); 
  ...
  try {
  this.connection.query(`SELECT seq, recipeName, rarity, summary from Recipe WHERE recipeName LIKE '%${recipe}%'`, (error:mysql.MysqlError, rows:any) => { //LIKE를 이용해 검색 
    if(error) { //에러 발생
      logs_(error.toString());
   ...
   return
    ...
    getAll = (req:Request, res:Response, next:NextFunction) => {
        try{
            this.connection.query(`SELECT seq, recipeName,rarity,summary,path from Recipe`, (error:mysql.MysqlError, rows:any) => { //쿼리
              if (error) { // 에러
                logs_(error.toString())
    ...
    addDetail = (req:Request, res:Response, next:NextFunction) => {
        try{
            ...
            this.connection.query(`INSERT INTO Recipe_Detail (recipeName, detail, VidPath, ImgPath) 
    ...
    getDetail = (req:Request, res:Response, next:NextFunction) => {
        try{
            this.connection.query(`SELECT * FROM Recipe_Detail WHERE recipeName='${req.params.recipeName}'` ,(error:mysql.MysqlError, rows:any) => {
